...
const DEFAULT_DEPLOYMENTS = [
  ...
  { service: 'nexus-worker',     env: 'Staging',     status: 'BUILDING', version: 'v2.4.2', deployedBy: 'Hamsha HM' },
  { service: 'nexus-db-migrate', env: 'Production',  status: 'FAILED',   version: 'v1.9.0', deployedBy: 'CI/CD Pipeline' },
  { service: 'nexus-cache',      env: 'Staging',     status: 'LIVE',     version: 'v1.3.0', deployedBy: 'Hamsha HM' },
];
...
export async function GET(request: Request) {
  try {
    ...
    // Auto-seed if Deployment model table is accessible and empty
    try {
      ...
      // If table hasn't been pushed yet, return static fallback cleanly
    }
...
export async function PATCH(request: Request) {
  try {
    ...
    let updated: any;
    try {
      updated = await (prisma as any).deployment.update({
        where: { id },
        data: { status }
      });
    } catch {
